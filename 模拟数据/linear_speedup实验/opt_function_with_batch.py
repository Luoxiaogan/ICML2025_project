...
def PullDiag_GD_with_batch(
    ...
    # 初始化梯度（使用批次）
    g = grad_func(x, y, h, rho=rho, batch_size=batch_size) + sigma_n * np.random.normal(size=(n, d))
    ...
    for _ in range(max_it):
        ...
        x_mean_expand = np.broadcast_to(x_mean, (n, d))
        _grad = grad_func(x_mean_expand, y, h, rho=rho, batch_size=None).reshape(x.shape)
        # 在整个训练数据集上计算梯度
    ...
    return pd.DataFrame({"gradient_norm_on_full_trainset": gradient_history_onfull, "x_mean_norm": x_history, "gradient_norm_on_batch": gradient_history})
...
def PullDiag_GT_with_batch(
    ...
    # 初始化梯度（使用批次）
    g = grad_func(x, y, h, rho=rho, batch_size=batch_size).reshape(x.shape) + sigma_n * np.random.normal(
        size=(n, d)
    )
    w = np.diag(1 / np.diag(W)) @ g
    v = copy.deepcopy(g)
    ...
    x_mean = np.mean(x, axis=0, keepdims=True)  # 1*d
    x_mean_expand = np.broadcast_to(x_mean, (n, d))  # n*d
    gradient_x_mean = grad_func(x_mean_expand, y, h, rho=rho, batch_size=None).reshape(x.shape)
    mean_gradient_x_mean = np.mean(gradient_x_mean, axis=0, keepdims=True)  # 1*d
    ...
    result_df = pd.DataFrame(
        {
            "gradient_norm": gradient_history_onfull,
            "gradient_tracking_norm": v_history,
